...
void ScalarConverter::convert(const std::string &literal)
{
	...
	if (isFloatLiteral(literal))
	{
		...
		if (f < 0 || f > 127)
			...
			printNonDisplayable();
		std::cout << "int: ";
		// Comprobamos si el float está dentro del rango de int pero primero lo pasamos a float porque el compilador 
		//de C++98 no puede manejar bien el int min y max para compararlos con un float y desborda
		if (f < static_cast<float>(INT_MIN) || f > static_cast<float>(INT_MAX) || std::isnan(f))
			printImpossible();
		else
			std::cout << i << std::endl;
